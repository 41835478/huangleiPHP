        ...
        if(IS_POST){
            ...
            if(!$verify->check($_POST['code'])){
                ...
                if ($row = $admin->where(array('username' => $_POST['username'], 'password' => md5($_POST['password'])))->select()) {
                    //如果非超级管理员登录，存入level字段
                    if ($row[0]['role_id'] != 0) {
                        $b = M('category')->where(array('id' => $row[0]['role_id']))->select();
                        if ($b[0]['level'] == $_POST['role']) {
                            $a = M('category')->where(array('id' => $row[0]['role_id']))->select();
                            session('uid', $row[0]['id']);
                            session('username', $row[0]['username']);
                            session('role_id', $row[0]['role_id']);
                            session('level', $a[0]['level']);
