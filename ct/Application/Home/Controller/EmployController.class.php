...
class EmployController extends CommonController
{
    ...
    public function add(){
        if(IS_POST){
            ...
            if($model->data($arr)->add()){
                $this->success('添加成功',U(MODULE_NAME.'/Employ/lst'));
            }else{
                $this->error('添加失败,请重试！');
            }
        }else{
    ...
    public function save(){
        ...
        if(IS_POST){
            if($model->create()){
                if($model->save() !== false){
                    $this->success('修改成功',U(MODULE_NAME.'/Employ/lst'));
                    exit;
                }else{
                    $this->error('修改失败,请重试！');
                }
            ...
            $this->data = M('Employ')->find($id);
            $this->display();
        }
    }
    //列表
    public function lst(){
        $this->data = M('Employ')->select();
        $this->display();
    }
    //删除
    public function del(){
