...
class BuildModel extends Model
{
    protected function _after_insert(&$data,$option){
        if(isset($_FILES['build']) && $_FILES['build']['tmp_name']){
            /************   添加图片到一个指定位置**********************/
            ...
            $upload->savePath  =      'Build/'; // 设置附件上传目录
            $info = $upload->upload();
            ...
            foreach($info as $v){
                ...
                $model->add(array('build_id'=>$data[0]['id'],'images'=>$images,'sm_images'=>$smimages,'big_images'=>$bigimages));
            }
        }
    }
    protected function _before_update($data,$option){

        if(isset($_FILES['build']) && $_FILES['build']['tmp_name'][0]){
            ...
            foreach($info as $v){
                ...
                $image = new \Think\Image();    //实例化
                $image->open('./Uploads/'.$images);
                $image->thumb(200, 150)->save('./Uploads/'.$bigimages);
                $image->thumb(50, 50)->save('./Uploads/'.$smimages);
                $model->add(array('build_id'=>$data[0]['id'],'images'=>$images,'sm_images'=>$smimages,'big_images'=>$bigimages));
            }
        }
    ...
    protected function _before_delete($options){
        ...
        if(is_array($options['where']['id'])){
            ...
            $child = M('build_images')->field('images,sm_images,big_images')->where("build_id IN($id)")->select();
            //遍历删除
            foreach($child as $v){
            ...
            $child = M('build_images')->field('images,sm_images,big_images')->where("build_id=".$options['where']['id'])->select();
            foreach($child as $v){
                @unlink('./Uploads/'.$v['images']);
                @unlink('./Uploads/'.$v['sm_images']);
                @unlink('./Uploads/'.$v['big_images']);
