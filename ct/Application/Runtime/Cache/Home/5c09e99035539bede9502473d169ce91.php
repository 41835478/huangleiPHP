...
<div class="w980">
  ...
  <div class="page_r" id="case">
      <div class="box_r">
          <div class="box_r_m" style="background:rgba(255,255,255,0.7); border-radius:15px;width:735px;">
              <div class="box_r_title"><p id="qqqq" style="color:rgba(121,190,247,1.00); font-size:25px;"><?php echo $catName['name'] ? $catName['name'] : '案例展示';?><span style="color:rgba(121,190,247,1.00); font-size:25px;">/Case Shows</span></p></div>
          </div>
      </div>
      <?php if(!empty($case)){ foreach($case as $v): ?>
      <div class="box_r mtop10">
          <div class="anli348">
              <div class="anli348_tu">
                  <a class="a" pid="<?php echo $v['id']?>" target="_self"><img src="/ct/Uploads/<?php foreach($img as $k){ if($k['case_id'] == $v['id']){ echo $k['big_images']; break; } }?>" width="212" height="148" border="0"></a>
              </div>
              <div class="anli348_title"><?php echo $v['name']?></div>
          </div>
      </div>
      <?php endforeach;?>
      <?php }else{?>
    ...
    <!-- 案例展示 详细内容-->
    <div class="page_r" id="content" style="display:none">
        <div class="box_r">
            <div class="box_r_m" style="background:rgba(255,255,255,0.7); border-radius:15px;width:735px;height:35px;">
                <div class="box_r_title"><p style="color:rgba(121,190,247,1.00); font-size:18px;" id="pp"><a href="<?php echo U(MODULE_NAME.'/Index/cas');?>"></a></p></div>
            </div>
        </div>

        <div class="box_r mtop10" id="img">
...
<script>
    $(document).ready(function(){
        ...
        $('.a').click(function(){
            var q =$(this).parents('.anli348_tu').next('.anli348_title').text();
            var k = $('#qqqq').text().split('/');
            var str = k[0]+'>>>>';
            var x = $(this).attr('pid');      //获取a标签中pid属性
            $.ajax({
                type:'get',
                url:"/ct/index.php/Home/Index/caseShow/id/"+x,
                success:function(data){
                    //$('#content').html(data.content);
                    //$('#cont').show().prevAll('.page_r').hide();
                    $.each(data,function(name,value){
                        $('#case').hide();
                        $('#content #img').append("<img src=/ct/Uploads/"+value.big_images+"><br/><br/>").css('margin-left','50px');
                    });
                }
            });
            $('#pp a').html(str);
