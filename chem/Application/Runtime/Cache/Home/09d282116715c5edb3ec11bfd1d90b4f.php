...
<h1>
    ...
    <span id="search_id" class="action-span1"> - 待审批列表 </span>
...
<form method="post" action="<?php echo U(MODULE_NAME.'/rules/bdel');?>" name="listForm">
 ...
 if($v['status'] == 1){ echo '管理员'; }else if($v['status'] == 2){ echo '主管'; }else if($v['status'] == 3){ echo '老师'; }else if($v['status'] == 4){ echo '已拒绝'; }else if($v['status'] == 0){ echo '已通过'; } ?>
                ...
                <a onclick="return confirm('确定要同意吗？');" href="<?php echo U(MODULE_NAME.'/Apply/agree',array('id'=>$v['id'],'stock_id'=>$v['stock_id']));?>" title="同意">同意</a>
                |
                <a onclick="return confirm('确定要拒绝吗？');" href="<?php echo U(MODULE_NAME.'/Apply/refuse',array('id'=>$v['id']));?>" title="拒绝">拒绝</a>
