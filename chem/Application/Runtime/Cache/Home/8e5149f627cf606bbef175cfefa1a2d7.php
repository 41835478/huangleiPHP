...
<h1>
    ...
    <span id="search_id" class="action-span1"> - 结算清单 </span>
    <div style="clear:both"></div>
</h1>
<div class="form-div">
    <form action="<?php echo U(MODULE_NAME.'/Apply/clearlst');?>" name="searchForm" method="get">
...
<form method="post" action="<?php echo U(MODULE_NAME.'/Apply/bdel');?>" name="listForm">
    <div class="list-div" id="listDiv">
        <table cellpadding="3" cellspacing="1">
            ...
            <?php foreach($data as $v): if($v['status']==1) continue; $price +=$v['price']; $count += $v['stock']; ?>
            <tr>
                <td align="center">
 ...
 foreach($goods as $k){ if($v['stock_id'] == $k['id']){ echo $k['goods_name'].'('.$k['size'].')'; } } ?>
                ...
                <td align="center">
                    <a onclick="return confirm('确定要结算吗？');" href="<?php echo U(MODULE_NAME.'/Apply/del',array('id'=>$v['id'],'role_id'=>$v['role_id']));?>" title="结算">结算</a>
                </td>
            <?php endforeach;?>
            <tr>
                <td><input onclick="return confirm('确定要结算吗？');" type="submit" value="结算所选" /></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
